...
function I_rot = my_rotation(I, radius);
...
% from the source image, and save to R_rot, G_rot, B_rot.
 for y_new = 1 : height_new
     for x_new = 1 : width_new
        ...
        tmp = matrix_inv*[x_new-x_shift ; y_new-y_shift];
        x_old = tmp(1);
        y_old = tmp(2);
        ...
        R_rot(y_new, x_new) =uint8(r);% r/255;
        G_rot(y_new, x_new) =uint8(g);% g/255;
        B_rot(y_new, x_new) =uint8(b);% b/255;
    end
 end
